...
const InfraredCardComponent: React.FC<Props> = ({ firebaseDb }) => {
  const classes = useStyles()
  const [temperatures, setTemperatures] = useState<Temperature[]>([])
  const [warning, setWarning] = useState<boolean>(false)

  const temperatureRef = ref(firebaseDb, 'rx_module0/infrared light');
  useEffect(() => {
  ...
  return (
    <DataCardComponent title="Infrared Light">
...
export default InfraredCardComponent
