...
const TemperatureCardComponent: React.FC<Props> = ({ firebaseDb }) => {
  const classes = useStyles()
  const [temperatures, setTemperatures] = useState<Temperature[]>([])
  const [warning, setWarning] = useState<boolean>(false)

  const temperatureRef = ref(firebaseDb, 'rx_module0/temp');
  useEffect(() => {
  ...
  return (
    <DataCardComponent title="Temperature">
      <ChartComponent temperature data={temperatures.map(t => t.value)} timestamps={temperatures.map(t => t.timestamp)} temperatureData={temperatures} />
    </DataCardComponent>
  )
}
...
export default TemperatureCardComponent
